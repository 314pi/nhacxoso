...
iAd_ChangeBanner = function(c, strDiv, arBanner)
{
    if (arBanner.length==0)
	    return;
	eval (c + "=" + c + "+1;");
    var counter = eval (c);
    if (counter >= arBanner.length)
    {
	    counter=0;
	    eval (c + "=0;");
	}
    var d = document.getElementById(strDiv); 
    var AdMode=arBanner[counter][4]; 
    var strLink=arBanner[counter][1]; 
    var strWidth=arBanner[counter][2]; 
    var strHeight=arBanner[counter][3]; 
    var strImgUrl=arBanner[counter][0]; 
    if (AdMode == 0) 
    {
    ...
    else if (AdMode == 1) 
    {
        ...
        d.innerHTML = "<object         classid='clsid:d27cdb6e-ae6d-11cf-96b8-444553540000'   codebase='http://fpdownload.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=7,0,0,0'  width=" + strWidth + "   height=" + strHeight + "  id=" + flash + "     align='middle'>    <param name='allowScriptAccess' value='sameDomain' /> <param name='movie' value='"+ strImgUrl + "'/>  <PARAM  NAME=FlashVars VALUE='link="+ strLink + "' >  <param  name='quality' value='high' />  <param  name='allowscriptaccess' value='always' /> <param  name='wmode' value='opaque' />   <embed wmode='opaque'  src='" + strImgUrl + "'   quality='high' width=" + strWidth + "   height=" + strHeight + "                 name=' helloWorld '                   flashvars='link=" + strLink + "'                   align='middle'                   allowfullscreen='true' allowscriptaccess='always'                   type='application/x-shockwave-flash'                   pluginspage='http://www.macromedia.com/go/getflashplayer' />           </object>  ";   
    }
}

iAd_DisplayBanner = function(c, arBanner, strDiv)
{
    iAd_ChangeBanner (c, strDiv,arBanner);
    if (arBanner.length>1)	
        setInterval( function () { iAd_ChangeBanner(c, strDiv, arBanner);}, 15000);
}

iAd_ShowBanner = function(strBanner, strDiv)
{  
    var d = document.getElementById(strDiv);
    d.innerHTML = strBanner;   
}
iAd_ShowPopUp = function(strCode, strDiv) {
